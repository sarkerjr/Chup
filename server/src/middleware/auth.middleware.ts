...
export default function (req: Request, _res: Response, next: NextFunction) {
  ...
  if (!token) {
    throw new ApiError(401, 'ERR_UNAUTHORIZED', true, 'Token is missing');
  }

  // Verify Token
  return verify(token, process.env.JWT_SECRET_KEY!, (error, decoded: any) => {
    if (error?.name === 'TokenExpiredError') {
      throw new ApiError(401, 'ERR_UNAUTHORIZED', true, 'Token is expired');
    }

    //Check if token is valid
    if (error) {
      throw new ApiError(401, 'ERR_UNAUTHORIZED', true, 'Token is invalid');
    }
